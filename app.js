...
const createTable = (vals) => {
  var headers = ["Name", "Genus", "Sugar"];
  var table = document.createElement("TABLE"); //makes a table element for the page

  for (var i = 0; i < vals.length; i++) {
    var row = table.insertRow(i);
    row.insertCell(0).innerHTML = vals[i].name;
    row.insertCell(1).innerHTML = vals[i].genus;
    row.insertCell(2).innerHTML = vals[i].nutritions.sugar;
  }
...
const updatePage = async () => {
  ...
  // Use either `map` and/or `filter` to extract some data from the array of fruit objects
  // For example, find "name of all fruits whose sugar > 15",
  result = fruitsArray.filter((fruit) => {
